...
#         response = APIClient().post(self.endpoint, data = Test_user_create.signup_info("ht_buses_app/tests/resources/users/create_user_parent.json"), format='json')
#         assert response.status_code == 200 # Checks that the response code is successful

#     def test_create_user_admin(self):
#         response = APIClient().post(self.endpoint, data = Test_user_create.signup_info("ht_buses_app/tests/resources/users/create_admin_user.json"), format='json')
#         assert response.status_code == 200 # Checks that the response code is successful
        
#     def test_create_user_admin_parent(self):
#         Test_user_create.create_school_route()
#         response = APIClient().post(self.endpoint, data = Test_user_create.signup_info("ht_buses_app/tests/resources/users/create_user_admin_parent.json"), format='json')
